...
- S1/Pad1：进入全屏 GIF 播放页面，S1/Pad1 和 S2/Pad2 在该页面切换上一个/下一个动图。
- S2/Pad2：进入 SD 卡文本阅读页面，S1/Pad1 和 S2/Pad2 在该页面翻到上一页/下一页。
- S3：进入设置界面。
- S4：手动执行一次全屏刷新。

设置选择界面：

- S1/Pad1 和 S2/Pad2：上下选择参数。
- S4：进入当前参数编辑。
- S3：返回主界面。
...
- S3：P2.3，内部上拉，按下为低电平。
- S4：P2.6，内部上拉，按下为低电平。
- Pad1/Pad2：P6.0/P6.1，使用 Comparator B 做电容触摸测频，功能分别等同 S1/S2。
